...
public class CollisionDetector {
    ...
    public CollisionDetector(BucketGenerator map, BallManager ballManager, int score, String password,List<PegGenerator.Peg> pegs) {
        this.bucket = map;
    ...
    public void sortAndTransform() {
        synchronized (newest) {
            if (!newest.isEmpty()) {
                ...
                try (BufferedWriter writer = new BufferedWriter(new FileWriter("entry.txt", true))) {
        ...
        try {
            List<String> lines = Files.readAllLines(Paths.get("entry.txt"));
            ...
            Files.write(Paths.get("output.txt"), transformedLines);
            System.out.println("Sorted and transformed lines written back to output.txt");

            // Delete the entry.txt file
            Files.deleteIfExists(Paths.get("entry.txt"));
        } catch (IOException e) {
